...
def lap_times_mod_time(file_name, tries_count):
    if tries_count > 30:
        log('Cannot find file, please make sure that the file is named exactly "CurrentPassings.csv" \nRestarting')
    ...
    try:
        ...
        if tries_count % 5 == 0:
            log('Waiting for {filename} export...'.format(filename=file_name))
        tries_count += 1
        time.sleep(2)
...
def start_race(restart):
    if restart:
        ...
        log('Please export a passings csv and name it EXACTLY: "{filename}"'.format(filename=file_name))
        time.sleep(2)
        log('You have 1 Minute'.format(filename=file_name))
        if abs(lap_times_mod_time(file_name, 0) - time.time()) < 3:
            ...
            log("{filename} is too old. Deleting {filename}, wait for prompt to export".format(filename=file_name))
            os.remove(file_name)
            start_race(restart)
    else:
        start_monitors(restart)
